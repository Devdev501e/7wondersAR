...
public class CardDecks {
	...
	public CardDecks(String wonder) {
		switch (wonder) {
			case "Halicarnasse":
				createDecks(wonder, deckCardQuantities_Halicarnasse);
				break;
			case "Alexandrie":
				createDecks(wonder, deckCardQuantities_Alexandrie);
				break;
			case "Babylone":
				createDecks(wonder, deckCardQuantities_Babylon);
				break;
			case "Olympie":
				createDecks(wonder, deckCardQuantities_Olympie);
				break;
			case "Rhodes":
				createDecks(wonder, deckCardQuantities_Rhodes);
				break;
			case "Ephese":
				createDecks(wonder, deckCardQuantities_Ephese);
				break;
			case "Gizeh":
				createDecks(wonder, deckCardQuantities_Gizeh);
				break;
			case "Main":
				createDecks("Main", deckCardQuantities_Extra);
				break;
		}
	}

	private void createDecks(String wonder, List<CardTypeQuantity> deckCardsQuantities) {
		for (CardTypeQuantity i : deckCardsQuantities) {
			for (int j = 0; j < i.quantity; j++ ) {
				if (wonder.equals("Main")) {
					cardDeck.add(new Card(i.cardType, CardBack.CentralDeck));
				}
				else {
					cardDeck.add(new Card(i.cardType, CardBack.valueOf(wonder)));
				}
			}
		}
	}
